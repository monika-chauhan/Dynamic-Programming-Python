...
def unboundedKnapsack(ind, weight, value, W, n):
    ...
    notPick = unboundedKnapsack(ind - 1, weight, value, W, n)
    pick = float('-inf')
    if weight[ind] <= W:
        pick = value[ind] + unboundedKnapsack(ind, weight, value, W - weight[ind], n)
...
print("The Maximum value of items, thief can steal is", unboundedKnapsack(n-1,wt,val,W, n))
