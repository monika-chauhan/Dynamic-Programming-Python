...
def maxPricebyRodCutting(ind, price, N):
    if ind == 0:
        return N * price[0]

    notPick = 0 + maxPricebyRodCutting(ind - 1, price, N,)
    pick = float('-inf')
    rodLength = ind + 1
    if rodLength <= N:
        pick = price[ind] + maxPricebyRodCutting(ind, price, N - rodLength)

    return max(pick,notPick)

price = [2, 5, 7, 8, 10]
n = len(price)
print("The Maximum price generated is", maxPricebyRodCutting(n-1, price, n))
