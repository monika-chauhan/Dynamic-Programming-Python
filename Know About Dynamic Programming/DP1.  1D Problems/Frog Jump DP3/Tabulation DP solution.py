...
def minEnergy_frog_jump(height, n):
    dp = [-1] * (n + 1)
    dp[0] = 0
    for ind in range(1, n):
        oneJump = dp[ind - 1] + abs(height[ind] - height[ind - 1])
        twoJump = float('inf')
        if ind > 1:
            twoJump = dp[ind - 2] + abs(height[ind] - height[ind - 2])
...
n = len(stair)
print(minEnergy_frog_jump(stair, n))
